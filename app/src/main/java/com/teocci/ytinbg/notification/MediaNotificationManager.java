...
public class MediaNotificationManager extends BroadcastReceiver
{
    ...
    private final BackgroundExoAudioService backgroundExoAudioService;
...
//    private final int mNotificationColor;
    ...
    {
        backgroundExoAudioService = service;
        updateSessionToken();

//        mNotificationColor = ResourceHelper.getThemeColor(backgroundExoAudioService, R.attr.colorPrimary,
//                Color.DKGRAY);

        notificationManager = NotificationManagerCompat.from(service);

        String pkg = backgroundExoAudioService.getPackageName();
        pauseIntent = PendingIntent.getBroadcast(backgroundExoAudioService, REQUEST_CODE,
                new Intent(ACTION_PAUSE).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        playIntent = PendingIntent.getBroadcast(backgroundExoAudioService, REQUEST_CODE,
                new Intent(ACTION_PLAY).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        previousIntent = PendingIntent.getBroadcast(backgroundExoAudioService, REQUEST_CODE,
                new Intent(ACTION_PREV).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        nextIntent = PendingIntent.getBroadcast(backgroundExoAudioService, REQUEST_CODE,
                new Intent(ACTION_NEXT).setPackage(pkg), PendingIntent.FLAG_CANCEL_CURRENT);
        mStopCastIntent = PendingIntent.getBroadcast(backgroundExoAudioService, REQUEST_CODE,
    ...
    {
        if (!hasStarted) {
            currentYouTubeVideo = backgroundExoAudioService.getCurrentYouTubeVideo();
            ...
            if (notification != null) {
                ...
                backgroundExoAudioService.registerReceiver(this, filter);

                backgroundExoAudioService.startForeground(NOTIFICATION_ID, notification);
                hasStarted = true;
            }
        }
    }
    ...
    public void stopNotification()
    {
        if (hasStarted) {
            ...
            try {
                ...
                backgroundExoAudioService.unregisterReceiver(this);
            } catch (IllegalArgumentException ex) {
                // ignore if the receiver is not registered.
            }
            backgroundExoAudioService.stopForeground(true);
        }
    }
    ...
    {
        MediaSessionCompat.Token freshToken = backgroundExoAudioService.getSessionToken();
        if (sessionToken == null && freshToken != null ||
            ...
            if (sessionToken != null) {
                mediaController = new MediaControllerCompat(backgroundExoAudioService, sessionToken);
    ...
    {
        Intent openUI = new Intent(backgroundExoAudioService, MainActivity.class);
        ...
        return PendingIntent.getActivity(backgroundExoAudioService, REQUEST_CODE, openUI,
    ...
    private final MediaControllerCompat.Callback callback = new MediaControllerCompat.Callback()
    {
        ...
        {
            MediaNotificationManager.this.currentYouTubeVideo = backgroundExoAudioService.getCurrentYouTubeVideo();
    ...
    private Notification createNotification()
    {
        ...
        Intent clickIntent = new Intent(backgroundExoAudioService, MainActivity.class);
        clickIntent.setAction(Intent.ACTION_MAIN);
        clickIntent.addCategory(Intent.CATEGORY_LAUNCHER);
        PendingIntent clickPendingIntent = PendingIntent.getActivity(backgroundExoAudioService, 0, clickIntent, 0);

        NotificationCompat.Builder notificationBuilder = new NotificationCompat.Builder(backgroundExoAudioService);
        int playPauseButtonPosition = 0;

        // If skip to previous action is enabled
        if ((playbackState.getActions() & PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS) != 0) {
            notificationBuilder.addAction(R.drawable.ic_skip_previous_white_24dp,
                    backgroundExoAudioService.getString(R.string.action_previous), previousIntent);
...
//                .setColor(mNotificationColor)
    ...
    private void addPlayPauseAction(NotificationCompat.Builder builder)
    {
        ...
        if (playbackState.getState() == PlaybackStateCompat.STATE_PLAYING) {
            label = backgroundExoAudioService.getString(R.string.action_pause);
            icon = R.drawable.ic_pause_white_24dp;
            intent = pauseIntent;
        } else {
            label = backgroundExoAudioService.getString(R.string.action_play);
